...
class WaybarAppsEditor:
    ...
    def load_config(self):
        try:
            if not os.path.exists(CONFIG_PATH):
                # Buat file kosong jika tidak ada (opsional)
                self.config = {}
                return
    ...
    def create_widgets(self):
        tk.Label(self.root, text="Waybar Image Shortcuts (GUI)", font=("JetBrainsMono Nerd Font", 14, "bold"), 
    ...
    def refresh_app_list(self):
        ...
        self.entries = {}
        # Cari module yang bertipe image (image#nama atau image)
        ...
        for key in image_keys:
            ...
            tk.Label(row, text=name[:10], fg="#8be9fd", bg="#2d3758", width=10, anchor="w").pack(side="left")

            # Path Entry (Tampilkan path gambar)
            val_path = tk.StringVar(value=self.config[key].get("path", ""))
            entry_path = tk.Entry(row, textvariable=val_path, bg="#1c2238", fg="white", width=30)
            entry_path.pack(side="left", padx=5)
            
            # Tombol Browse ulang untuk mengganti gambar
    ...
    def browse_image(self, var):
        filename = filedialog.askopenfilename(title="Pilih Icon", 
                                            filetypes=[("Image files", "*.png *.svg *.jpg *.jpeg")])
        if filename:
            var.set(filename)

    def add_app_gui(self):
        name = simpledialog.askstring("Input", "Nama Aplikasi:")
        if not name: return
        
        img_path = filedialog.askopenfilename(title="Pilih Gambar untuk " + name, 
                                            filetypes=[("Image files", "*.png *.svg *.jpg")])
        if img_path:
            ...
            # Tambahkan ke urutan modules (misal modules-left)
            # Pastikan 'modules-left' atau 'modules-right' ada di config kamu
            target_group = "modules-left" 
            if target_group in self.config:
                if key not in self.config[target_group]:
                    self.config[target_group].append(key)
    ...
    def save_config(self):
        ...
        try:
            ...
            messagebox.showinfo("Sukses", "Waybar berhasil di-reload dengan gambar!")
        except Exception as e:
