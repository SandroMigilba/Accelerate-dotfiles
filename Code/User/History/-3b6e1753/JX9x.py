...
# Memastikan folder config ada agar tidak error saat dijalankan
...
class CyberWaybarEditor:
    def __init__(self, root):
        ...
        self.bg_deep = "#0b0e14"
        self.card_bg = "#151921"
        self.input_bg = "#1c232d"
        self.accent_primary = "#1793d1"
        self.accent_secondary = "#bb9af7"
        self.text_main = "#c0caf5"
    ...
    def load_config(self):
        try:
            if not os.path.exists(CONFIG_PATH):
                # Template default jika file belum ada
                self.config = {"group/apps": {"modules": []}}
                return
            with open(CONFIG_PATH, 'r') as f:
                content = f.read()
                # Menghapus komentar // agar JSON valid
            ...
            messagebox.showwarning("Warning", f"Config baru akan dibuat karena: {e}")
            self.config = {"group/apps": {"modules": []}}

    # ... (Sisa fungsi create_widgets, create_btn, dll sama dengan sebelumnya)
    # Masukkan sisa kode dari jawaban saya sebelumnya di sini
