...
class WaybarListManager:
    ...
    def col(self, parent, text, col):
        tk.Label(
            ...
            bg=self.bg,
            fg=self.sub
        ).grid(row=0, column=col, sticky="w", padx=(8, 12))
    ...
    def add_app(self):
        ...
        lb = tk.Listbox(
            win, bg=self.row, fg=self.text,
            selectbackground=self.blue,
            relief="flat"
        )
        lb.pack(fill="both", expand=True, padx=16, pady=16)
        ...
        self.btn(win, "Add", self.blue, pick).pack(pady=16)
