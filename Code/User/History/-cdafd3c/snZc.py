...
class GlassBT(QWidget):
    ...
    def start_scan(self):
        if not self.is_scanning:
            ...
            # Gunakan subprocess.run dengan capture_output agar tidak membanjiri terminal
            subprocess.run(["bluetoothctl", "scan", "on"], capture_output=True)
            
            # Timer untuk mematikan scan otomatis agar tidak konflik
            # Gunakan interval yang lebih pendek atau pastikan hanya dipanggil sekali
            self.scan_timer = QTimer.singleShot(15000, self.stop_discovery)
            self.refresh_devices()

    def stop_discovery(self):
        if self.is_scanning:
            # Menggunakan check=False agar jika gagal (karena sudah mati), skrip tidak crash
