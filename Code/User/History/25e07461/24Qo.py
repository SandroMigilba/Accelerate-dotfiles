...
class GlassWifi(QWidget):
    def __init__(self):
        super().__init__()
        self.is_connecting = False  # Flag agar tidak refresh saat input password
        self.initUI()
    ...
    def connect_wifi(self, name):
        ...
        self.header_label.setText(f"Waiting for Password...")
        
        # Perintah agar Alacritty terbuka, menjalankan nmcli, dan tetap terbuka jika ada error
        # Kita gunakan 'sh -c' agar perintah shell dieksekusi dengan benar
        cmd = f'alacritty -e sh -c "nmcli device wifi connect \'{name}\' --ask || read -p \'Press Enter to close...\'"'
        
        subprocess.Popen(cmd, shell=True)
        
        # Kembalikan status setelah 15 detik (estimasi waktu input password)
        QTimer.singleShot(15000, self.reset_connect_status)
