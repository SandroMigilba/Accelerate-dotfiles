...
class GlassWifi(QWidget):
    ...
    def get_wifi_data(self):
        try:
            # Memastikan hardware Wi-Fi aktif sebelum scan
    ...
    def connect_wifi(self, name):
        ...
        # Menghapus profil lama agar NM tidak bingung (Mencegah error 'never used')
        subprocess.run(["nmcli", "connection", "delete", name], check=False)
        
        # Eksekusi terminal untuk input password interaktif
        # Menggunakan format list untuk keamanan parsing spasi
        cmd = f'alacritty -e sh -c "nmcli device wifi connect \'{name}\' --ask || read -p \'Connection failed. Press Enter...\'"'
        subprocess.Popen(cmd, shell=True)
        
        # Memberikan jeda sebelum refresh agar status koneksi sempat berubah
