...
class WifiItem(QFrame):
    ...
    def init_item(self):
        ...
        self.label_info = QLabel(f"{'✧ ' if active else ''}{self.net['ssid']} ({sig}%)")
        self.label_info.setStyleSheet(f"color: {color}; font-weight: bold; background: transparent;")
        
        self.btn_expand = QPushButton("Connect" if not active else "Active")
        self.btn_expand.setFixedWidth(80)
        self.btn_expand.setStyleSheet("background: rgba(255,255,255,0.1); border: none; font-size: 10px;")
        self.btn_expand.clicked.connect(self.toggle_accordion)

        btn_del = QPushButton("🗑")
        btn_del.setFixedWidth(35)
        btn_del.setStyleSheet("background: rgba(255, 50, 50, 0.2); border: none;")
        ...
        btn_submit = QPushButton("Enter")
        btn_submit.setFixedWidth(60)
        btn_submit.setStyleSheet("background: #e6e6e6; color: black; font-weight: bold;")
...
class GlassWifi(QWidget):
    def __init__(self):
        ...
        self.signals.finished.connect(self.update_ui_list)
        self.initUI()
    ...
    def initUI(self):
        ...
        self.header_label.setStyleSheet("color: white; font-size: 18px; font-weight: bold; padding: 10px;")
        
        close_btn = QPushButton("✕")
        close_btn.setFixedSize(30, 30)
        close_btn.setStyleSheet("background: rgba(255, 80, 80, 0.4); border-radius: 15px; border: none;")
        ...
        self.scan_btn.setStyleSheet("background: rgba(255,255,255,0.1); padding: 12px; font-weight: bold; border-radius: 10px;")
        self.scan_btn.clicked.connect(self.start_refresh)
    ...
    def animate_loading(self):
        ...
        self.header_label.setText(f"Scanning {self.frames[self.f_idx]}")
        ...
        try:
            ...
            for line in out:
                ...
                if len(p) >= 4 and p[1].strip() and p[1] not in seen:
                    ...
                    seen.add(p[1])
            devs.sort(key=lambda x: x['signal'], reverse=True)
        except: pass
        self.signals.finished.emit(devs)

    def update_ui_list(self, networks):
    ...
    def execute_connect(self, ssid, password):
        ...
        QTimer.singleShot(5000, self.start_refresh)
