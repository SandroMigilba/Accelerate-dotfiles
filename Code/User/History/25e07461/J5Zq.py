...
class GlassWifi(QWidget):
    ...
    def initUI(self):
        ...
        self.setStyleSheet("""
            QWidget { background-color: rgba(15, 15, 15, 0.8); border-radius: 20px; color: white; }
    ...
    def start_scan_thread(self):
        # Update status agar user tahu skrip sedang bekerja
        if self.status.text() == "🌐 Network Manager":
            self.status.setText("🌐 Scanning...")
        threading.Thread(target=self.get_data, daemon=True).start()

    def get_data(self):
        try:
            # 1. Paksa rescan (tapi jangan tunggu lama)
            subprocess.run(["nmcli", "device", "wifi", "rescan"], check=False, capture_output=True)
            
            # 2. Ambil SSID Aktif
            self.active_ssid = ""
            active_out = subprocess.check_output(["nmcli", "-t", "-f", "ACTIVE,SSID", "dev", "wifi"]).decode().splitlines()
            for line in active_out:
                if line.startswith("yes:"):
                    parts = line.split(":")
                    if len(parts) > 1:
                        self.active_ssid = parts[1]

            # 3. Ambil List Wifi
            output = subprocess.check_output(["nmcli", "-t", "-f", "SSID,BARS,SIGNAL", "dev", "wifi", "list"]).decode().splitlines()
            
            networks = []
            seen = set()
            for line in output:
                p = line.split(':')
                # Pastikan SSID tidak kosong dan belum diproses
                if len(p) >= 3 and p[0].strip() and p[0] not in seen:
                    networks.append({"ssid": p[0], "bars": p[1], "sig": int(p[2] if p[2].isdigit() else 0)})
                    seen.add(p[0])
            
            # Balikkan ke UI thread
    ...
    def render_list(self, networks):
        ...
        if not networks:
            no_wifi = QLabel("No Wi-Fi detected.\nCheck if Wi-Fi is on.")
            no_wifi.setAlignment(Qt.AlignmentFlag.AlignCenter)
            self.list_layout.addWidget(no_wifi)
        else:
            for net in networks:
        ...
        self.status.setText("🌐 Network Manager")

    def switch_wifi(self, ssid):
        self.status.setText(f"🔄 Switching to {ssid}...")
        # Gunakan shell=True jika variabel mengandung spasi
        subprocess.Popen(f'alacritty -e nmcli device wifi connect "{ssid}" --ask', shell=True)
        QTimer.singleShot(6000, self.start_scan_thread)
