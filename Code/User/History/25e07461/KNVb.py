...
class GlassWifi(QWidget):
    ...
    def get_wifi_data(self):
        try:
            ...
            for line in output:
                parts = line.split(':')
                if len(parts) >= 4 and parts[1].strip() and parts[1] not in seen_ssids:
                    wifi_list.append({
                        "active": parts[0] == "yes",
                        "ssid": parts[1],
                        "bars": parts[2],
    ...
    def refresh_list(self):
        ...
        for net in networks:
            ...
            row_container.setStyleSheet(f"background-color: {bg_color}; border-radius: 10px;")
            
            row = QHBoxLayout(row_container)
            color = "#50fa7b" if net['signal'] > 75 else "#f1fa8c" if net['signal'] > 50 else "#ff5555"

            btn_text = f"{'⭐ ' if net['active'] else ''}{net['ssid']} ({net['signal']}%)"
            ...
            # Trigger koneksi saat diklik
            btn_conn.clicked.connect(lambda ch, n=net['ssid']: self.connect_wifi(n))
            
            btn_del = QPushButton("🗑")
            btn_del.setFixedWidth(40)
            btn_del.setStyleSheet("background-color: rgba(255, 50, 50, 0.2); border: none;")
    ...
    def connect_wifi(self, name):
        self.header_label.setText("Input Password...")
        # Menghapus profil lama agar memicu --ask dengan bersih
        subprocess.run(["nmcli", "connection", "delete", name], check=False)
        
        # Perintah terminal untuk meminta password
        # Gunakan alacritty -e sh -c agar jendela tetap terbuka dan menunggu input
        cmd = f'alacritty -e sh -c "nmcli device wifi connect \'{name}\' --ask || read -p \'Press Enter to close...\'"'
        subprocess.Popen(cmd, shell=True)
        
        # Refresh status setelah beberapa saat
        QTimer.singleShot(15000, self.refresh_list)
